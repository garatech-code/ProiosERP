  ...
  if (products.length === 0 && !editingId) {
    return (
      <div className="mt-4 p-4 border border-dashed border-gray-300 rounded-xl text-center">
        <p className="text-sm text-gray-500 mb-3">No se detectaron productos automáticamente.</p>
        <button onClick={addNewProduct} className="text-sm text-indigo-600 font-bold hover:text-indigo-800">
  ...
  return (
    <div className="mt-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 border-b border-gray-100 pb-4">
        ...
        <div className="flex items-center gap-4 mt-3 sm:mt-0 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 shadow-inner">
          <span className="text-xs font-bold text-gray-500 mr-2 uppercase">Buscador Principal:</span>
          ...
          <label className="flex items-center gap-1.5 cursor-pointer text-sm font-medium text-gray-700">
            <input 
              ...
              name="search_engine" 
              value="mercadolibre" 
              checked={searchEngine === 'mercadolibre'} 
              onChange={() => setSearchEngine('mercadolibre')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map((p) => {
          ...
          return (
            <div key={p.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow relative group flex flex-col justify-between">
              ...
              {isEditing ? (
                <div className="space-y-3 mb-4">
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Producto</label>
                    <input 
                      type="text" 
                      value={p.nombre} 
                      onChange={(e) => updateProduct(p.id, 'nombre', e.target.value)} 
                      placeholder="Ej. Botas solas"
                      className="w-full mt-1 border-gray-300 rounded-md text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                      autoFocus
                    />
                  </div>
                  <div className="flex gap-2">
                    <div className="w-1/3">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Cant.</label>
                      <input 
                        type="number" 
                        value={p.cantidad} 
                    ...
                    <div className="w-2/3">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Unidad</label>
                      <input 
                        type="text" 
                        value={p.unidad} 
                  ...
                  <button onClick={finishEditing} className="w-full py-2 bg-indigo-50 text-indigo-700 font-bold text-xs rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-200">
                    Guardar
                  </button>
                </div>
              ) : (
                <div className="mb-4 pr-6">
                  <h5 className="font-bold text-gray-900 leading-tight mb-1" title={p.nombre}>{p.nombre}</h5>
                  <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-xs font-semibold text-slate-700">
                    <i className="bi bi-tag-fill text-slate-400"></i> {p.cantidad} {p.unidad}
                  </div>
                  <button onClick={() => setEditingId(p.id)} className="text-[10px] uppercase font-bold text-indigo-500 hover:text-indigo-700 ml-3 underline decoration-indigo-200 underline-offset-2">
                    Editar
                  </button>
                </div>
              )}

              {/* Botones de acción (solo si no edita y hay nombre) */}
              {!isEditing && p.nombre && (
                <div className="mt-auto grid gap-2">
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="w-full text-center py-1.5 text-xs font-bold rounded-lg text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 transition-colors"
                  >
        ...
        <div 
          ...
          className="bg-transparent border-2 border-dashed border-gray-300 rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition-all min-h-[140px] opacity-70 hover:opacity-100"
        >
          <i className="bi bi-plus-circle text-2xl text-gray-400 mb-2"></i>
          <span className="text-sm font-bold text-gray-500">Añadir Otro Producto</span>
        </div>
      </div>
    </div>
  );
