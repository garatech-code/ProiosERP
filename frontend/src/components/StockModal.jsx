                      ...
                      {!stockData.todo_suficiente && (
                        <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                          <p className="text-red-800 text-sm font-medium">
                            ⚠️ Stock insuficiente para confirmar la operación.
                          </p>
                        </div>
                      )}
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {stockData.detalles.map((item, idx) => (
                                <tr key={idx} className={!item.suficiente ? 'bg-red-50' : ''}>
                                  ...
                                  <td className="px-4 py-3 text-sm text-gray-900">{item.cantidad_necesaria}</td>
                                  <td className="px-4 py-3 text-sm">
                                    <span className={!item.suficiente ? 'text-red-600 font-bold' : 'text-gray-900'}>
                                      {item.stock_actual}
                                    </span>
                                  </td>
                                  <td className="px-4 py-3">{getStockStatusBadge(item.suficiente)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
